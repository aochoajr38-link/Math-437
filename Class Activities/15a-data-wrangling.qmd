...
Hitters_Advanced <- readr::read_csv(here::here("C:/Users/aocho/Downloads/Hitters_Advanced.csv"))
...
set.seed(105)
Hitters_split <- initial_split(Hitters, prop = 0.80)
...
Hitters2 <- Hitters_train |> 
 dplyr :: select(Salary, AtBat, Hits, HmRun, League, Division, NewLeague)
```
...
Hitters3 <- Hitters_train |> # select the variables of interest
  dplyr :: select(Salary, AtBat, Hits, HmRun, League, Division, NewLeague) |>
  filter(AtBat >= 200)
```
...
Hitters3 |>
  arrange(
    desc(HmRun), # sort decending by homerun
    Salary
  ) |>
  dplyr :: select(Name, HmRun, Salary, everything()) |>
...
Hitters_left <- Hitters3 |>
  #everyone is Hitters3 is in hitters_left
  left_join(
    Hitters_Advanced,
...
```{r}
#| label: inner join
Hitters_inner_join <- Hitters3 |>
  inner_join(
    Hitters_Advanced,
    by = c("Name" = "PlayerName")
  )
view(Hitters_inner_join)
```
