...
pca_recipie <- recipe(
  ~.,# everything in this dataset is a predictor
  data = batting_rate_stats
) |>
  update_role(Name, new_role = "id") |>
  step_normalize(all_numeric_predictors()) |>
  step_pca(all_predictors(), num_comp =  10) #  num_comp, how many principle components do i want to keep to put into the model?
```
...
Goal: identify patterns in the data without explicitly predicting a response
...
...
x_l = e_l^T x
z_i 's are called the principle component score vectors
e_i .s are called loadings
...
BA PC1 = -0.335(BA) + (-0.4996)(SlugPct) +...+ 
...
pca_tidy |>
  ...
  facet_wrap(vars(component)) # PC1 and PC2 on different graphs
```
...
pca_pve # percentage of variation explained
```

```{r}
#| label: scree plots with ggplot2
ggplot(
  data = pca_pve |> filter( terms == "percent variance"),
